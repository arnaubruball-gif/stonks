...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="Alpha Quant v11.4 - Unified Sniper", layout="wide")
...
st.markdown("""
    ...
    .signal-card { background: linear-gradient(135deg, #1e2530 0%, #0d1117 100%); border: 2px solid #30363d; padding: 25px; border-radius: 15px; }
    </style>
...
if data is not None:
    ...
    with tab1:
        ...
        # LÓGICA DE SEÑAL + PROBABILIDAD UNIFICADA
        if abs(row['Z_Diff']) > 1.0 and row['R2'] > 0.05:
            # Cálculo de Probabilidad
            base_p = 50.0
            mod_z = abs(row['Z_Diff']) * 12
            mod_r2 = row['R2'] * 45
            prob = min(base_p + mod_z + mod_r2, 98.2)
            ...
            st.info("📉 Sistema en vigilancia: Esperando confluencia institucional (Z-Diff > 1.0 & R2 > 0.05)")
    ...
    st.error("Error al conectar con la API de mercados.")
