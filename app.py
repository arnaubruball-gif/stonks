...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="Alpha Quant v7.3 - Master Terminal", layout="wide")
...
if data is not None:
    ...
    tab1, tab2, tab3 = st.tabs(["📊 Gráfico & Camarilla", "🧬 Score Dual & Auditoría", "🕵️ Dirección Estadística"])
    ...
    with tab1:
        ...
        with st.expander("📖 ¿Cómo leer los Niveles Camarilla?"):
            st.write("Los niveles H3/L3 indican zonas de **reversión media**. H4/L4 indican zonas de **ruptura de tendencia**.")
    ...
    with tab2:
        ...
        if z_p > 2.0 and z_d > 1.4: color, msg = "#ff4b4b", "🚨 VENTA DE ALTA PROBABILIDAD (Confluencia Dual)"
        elif z_p < -2.0 and z_d < -1.4: color, msg = "#00cc96", "🟢 COMPRA DE ALTA PROBABILIDAD (Confluencia Dual)"
        elif abs(z_p) > 2.0: color, msg = "#ffa500", "⚠️ PRECIO SOBREEXTENDIDO (Falta confirmación de flujo)"
        else: color, msg = "#3d4463", "⚪ ESTADO: MERCADO EN EQUILIBRIO"
        ...
        fig_dual.add_trace(go.Scatter(x=data.index[-80:], y=data['Z_Diff'].tail(80), name="Z-Diff (RMF)", line=dict(color='#ffd700', dash='dot', width=2)))
        ...
        with st.expander("📖 ¿Qué es el Score Dual?"):
            st.write("El **Z-Price** mide la distancia del precio a su media (elástico). El **Z-Diff** mide si el flujo de dinero real acompaña al precio. La confluencia de ambos ocurre cuando las líneas azul y amarilla salen de las bandas grises al mismo tiempo.")

        st.divider()
        st.write("**🎯 Auditoría de Presión Histórica (7 Velas)**")
    ...
    with tab3:
        ...
        recent_rets = data['Ret'].tail(60).dropna()
        ...
        with ca:
            ...
            st.caption("Asimetría de retornos.")
        with cb:
            st.metric("Kurtosis (Inercia)", f"{kurt_val:.2f}")
            st.caption("Riesgo de colas anchas.")
        with cc:
            st.metric("Hurst Exponent", f"{hurst_val:.2f}")
            st.caption("Persistencia de tendencia.")

        with st.expander("📖 Guía de Inferencia"):
            st.markdown("""
            * **Skewness:** Si es positivo (>0), los retornos alcistas son más frecuentes. Negativo sugiere pánico.
            * **Kurtosis:** Valores > 3 indican que el activo es propenso a movimientos explosivos ("Cisnes Negros").
            * **Hurst:** > 0.55 es tendencia (momentum). < 0.45 es rango (mean reversion).
            """)

        st.divider()

        data['Expected'] = data['Ret'].rolling(20).mean()
        data['Residuals'] = data['Ret'] - data['Expected']
