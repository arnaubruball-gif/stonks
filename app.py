...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="Alpha Quant v10.9 - Camarilla Sniper", layout="wide")
...
def get_final_data(ticker_id, t):
    ...
    # --- CÁLCULO CAMARILLA (Basado en el día anterior) ---
    # Necesitamos OHLC del día anterior completo
    ...
    df['H4'] = C + range_val * (1.1 / 2)
    df['H3'] = C + range_val * (1.1 / 4)
    df['L3'] = C - range_val * (1.1 / 4)
    df['L4'] = C - range_val * (1.1 / 2)
...
assets = {
    ...
    "Commodities": {"Oro (Gold)": "GC=F", "Petróleo (WTI)": "CL=F"},
    "Indices": {"Nasdaq 100": "^IXIC", "S&P 500": "^GSPC", "DAX 40": "^GDAXI"},
    "Crypto": {"Bitcoin": "BTC-USD", "Ethereum": "ETH-USD"}
}

st.sidebar.title("📑 Master Sniper v10.9")
...
if data is not None:
    row = data.iloc[-1]
    # TABS: Se añade la 5 sin tocar las anteriores
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["🎯 Sniper Ejecución", "🕵️ Diagnóstico", "🧬 Historial Flujo", "🔗 Absorción", "🏰 Camarilla Levels"])

    with tab1:
        st.subheader(f"Plan Táctico - {nombre}")
        ...
        if abs(row['Z_Diff']) > 1.0 and row['R2'] > 0.05:
            ...
            st.markdown(f"<div class='diag-box' style='border-color:{color};'><h3>DISPARAR: {'LONG' if row['Z_Diff'] < -1.0 else 'SHORT'}</h3>"
                        f"Precio: <b>{row['Close']:.4f}</b></div>", unsafe_allow_html=True)
    ...
    with tab4:
        ...
        st.plotly_chart(px.bar(data.tail(50), y='Z_Eff', color='Z_Eff', color_continuous_scale='RdYlGn').update_layout(template="plotly_dark"), use_container_width=True)

    with tab5:
        st.markdown("<div class='gold-header'>🏰 PIVOTES CAMARILLA (DIARIOS)</div>", unsafe_allow_html=True)
        st.write("Úsalos como zonas de llegada (TP) o rebote estructural:")
        
        col1, col2, col3, col4 = st.columns(4)
        col1.markdown(f"<div class='camarilla-box'><b style='color:red;'>H4 (Breakout)</b><br>{row['H4']:.4f}</div>", unsafe_allow_html=True)
        col2.markdown(f"<div class='camarilla-box'><b style='color:orange;'>H3 (Reversión)</b><br>{row['H3']:.4f}</div>", unsafe_allow_html=True)
        col3.markdown(f"<div class='camarilla-box'><b style='color:lightgreen;'>L3 (Reversión)</b><br>{row['L3']:.4f}</div>", unsafe_allow_html=True)
        col4.markdown(f"<div class='camarilla-box'><b style='color:green;'>L4 (Breakout)</b><br>{row['L4']:.4f}</div>", unsafe_allow_html=True)
        
        # Gráfico con niveles Camarilla
